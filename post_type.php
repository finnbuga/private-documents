...
function otm_documents_set_bulk_document_updated_messages( $bulk_messages, $bulk_counts ) {
	$bulk_messages['document'] = array(
		...
		'locked'    => ( 1 == $bulk_counts['locked'] ) ? __( '1 document not updated, somebody is editing it.' ) :
			_n( '%s document not updated, somebody is editing it.',
				'%s documents not updated, somebody is editing them.', $bulk_counts['locked'] ),
		'deleted'   => _n( '%s document permanently deleted.', '%s documents permanently deleted.',
			$bulk_counts['deleted'] ),
		'trashed'   => _n( '%s document moved to the Trash.', '%s documents moved to the Trash.',
			$bulk_counts['trashed'] ),
		'untrashed' => _n( '%s document restored from the Trash.', '%s documents restored from the Trash.',
...
function otm_documents_set_document_updated_messages( $messages ) {
	...
	$messages['document'] = array(
		...
		5  => isset( $_GET['revision'] ) ? sprintf( __( 'Document restored to revision from %s.' ),
		...
		9  => sprintf( __( 'Document scheduled for: %s.' ),
				'<strong>' . $scheduled_date . '</strong>' ) . $scheduled_document_link_html,
